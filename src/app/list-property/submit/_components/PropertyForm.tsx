...
export default function PropertyForm({ plan }: Props) {
	const router = useRouter();
	const [isPending, startTransition] = useTransition();
	const fileInputRef = useRef<HTMLInputElement>(null);

	const {
		step, tag, type, beds, baths, sqft, title, location, description,
		price, amenities, images, previews, contactEmail, contactPhone,
		setField, toggleAmenity, addImages, removeImage,
	...
	const canProceed = () => {
		switch (step) {
			case 1: return !!tag;
			case 2: return !!type && !!beds && !!baths && !!sqft;
			case 3: return !!title && title.length >= 5 && !!location && !!description && description.length >= 50;
			case 4: return !!price && Number(price) > 0;
			case 5: return images.length > 0;
			case 7: return isValidEmail(contactEmail) && contactPhone.trim().length >= 7;
			default: return true;
		}
	};
	...
	return (
		<div className="max-w-2xl mx-auto px-4 sm:px-0">
			...
			<div className="bg-white border border-gray-200 rounded-2xl p-6 sm:p-8 shadow-sm min-h-80">
				...
				{step === 1 && (
					<div>
						<h2 className="text-xl font-bold text-[#0b1535] mb-1.5">What type of listing is this?</h2>
						<p className="text-sm text-gray-400 mb-8">Choose whether you are selling or renting out your property.</p>
				...
				{step === 2 && (
					<div>
						<h2 className="text-xl font-bold text-[#0b1535] mb-1.5">Property details</h2>
						<p className="text-sm text-gray-400 mb-8">Tell us about the property&apos;s physical characteristics.</p>
				...
				{step === 3 && (
					<div>
						<h2 className="text-xl font-bold text-[#0b1535] mb-1.5">Location &amp; description</h2>
						<p className="text-sm text-gray-400 mb-8">Give buyers and renters a clear picture of what makes your property special.</p>
						<div className="flex flex-col gap-5">
							<div>
							...
							<div>
								...
								<p className="text-xs text-gray-400 mt-1 text-right">
									...
									<span className={description.length < 50 ? "text-amber-500" : "text-green-500"}>
										(min 50)
									</span>
								</p>
							</div>
				...
				{step === 4 && (
					<div>
						<h2 className="text-xl font-bold text-[#0b1535] mb-1.5">Set your price</h2>
						<p className="text-sm text-gray-400 mb-8">
							Enter the asking price for your property.{" "}
							{tag === "For Rent" && "This will be shown as a per-year figure."}
						</p>
				...
				{step === 5 && (
					<div>
						<h2 className="text-xl font-bold text-[#0b1535] mb-1.5">Add photos</h2>
						<p className="text-sm text-gray-400 mb-6">
							Upload at least 1 photo. Listings with more photos get significantly more views.
						</p>
						...
						{images.length === 0 ? (
							...
							>
								...
								<span className="text-xs">PNG, JPG, WEBP — up to 10 MB each</span>
							</button>
						) : (
							<div>
								...
								<p className="text-xs text-gray-400">
									{images.length} photo{images.length !== 1 ? "s" : ""} added. First photo is the cover image.
								</p>
							</div>
						)}
						<input
				...
				{step === 6 && (
					<div>
						<h2 className="text-xl font-bold text-[#0b1535] mb-1.5">Amenities</h2>
						<p className="text-sm text-gray-400 mb-6">
							Select all amenities that apply. This helps buyers filter and find your property.
						</p>
				...
				{step === 7 && (
					<div>
						...
						<p className="text-sm text-gray-400 mb-8">
							Interested buyers and renters will use these details to reach you directly.
						</p>
						<div className="flex flex-col gap-5">
							<div>
								...
								<p className="text-xs text-gray-400 mt-1">
									Shown on your listing so buyers can email you directly.
								</p>
							</div>
							<div>
								...
								/>
								<p className="text-xs text-gray-400 mt-1">
									Include the country code for international visibility.
								</p>
							</div>

							<div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-700">
								<strong>Privacy note:</strong> Your contact details will only be visible to logged-in users viewing your listing.
							</div>
						</div>
					</div>
				)}
				...
				{step === 8 && (
					<div>
						<h2 className="text-xl font-bold text-[#0b1535] mb-1.5">Review your listing</h2>
						<p className="text-sm text-gray-400 mb-6">Everything look good? Hit submit to send your listing for review.</p>
						<div className="space-y-0 divide-y divide-gray-100 border border-gray-100 rounded-xl overflow-hidden">
							...
							<ReviewRow label="Details" value={`${beds} bed · ${baths} bath · ${sqft}`} />
							...
							/>
							{amenities.length > 0 && (
								<ReviewRow label="Amenities" value={amenities.join(", ")} />
							)}
							<ReviewRow label="Contact Email" value={contactEmail} />
							<ReviewRow label="Contact Phone" value={contactPhone} />
						</div>
			...
			<p className="text-center text-xs text-gray-400 mt-4">
				Your progress is saved automatically — safe to close and come back.
			</p>
		</div>
	);
}
...
function ReviewRow({ label, value }: { label: string; value: string }) {
	return (
		<div className="flex justify-between gap-4 px-4 py-3 text-sm bg-white">
			...
			<span className="text-[#0b1535] font-semibold text-right wrap-break-word max-w-[60%]">{value}</span>
		</div>
	);
}
